...
bool HardInteraction::IsResonance(int id) const {
	if (resonanceIds.size() > 0) {
		...
		return id == 32 || id == 23 || id == 39 || id == 5000039 || id == 24 || id == -24 || id == 34 ;
}

void HardInteraction::Clear() {
	quark = resonance = lepPlus = lepMinus = lepPlusNoIB = lepMinusNoIB = neutrino = antiNeutrino = 0;
	lepMinusAll.clear();
	lepPlusAll.clear();
...
bool HardInteraction::IsValidForNeutrinos() const { 
	return neutrino != 0 && antiNeutrino != 0;
}
bool HardInteraction::IsValidForAll() const { 
	return ((lepPlusAll[0] != 0 && lepMinusAll[0] != 0)/* || (neutrinoAll!=0 && lepPlusAll!=0) || (antiNeutrinoAll!=0 && lepMinusAll!=0) */) ;
}
...
void HardInteraction::Fill(const reco::GenParticleCollection& genParticles) {
	...
	for (; genp != genParticles.end(); genp++) {
		if (abs(genp->pdgId()) == 13 && genp->status()==1) counter+=1;
		//if (genp->status()==1){
		//std::cout << "Mother =  " << genp->mother()->pdgId() << std::endl;
		//std::cout << "status mother = " << genp->mother()->status() << std::endl;
		//if (genp->mother()->status()==2){
		//std::cout << "STATUS 2 MOTHER with ID =  " << genp->mother()->pdgId() << std::endl;
		//}	
		//if (genp->mother()->pdgId()==23) counter+=1;
		//}
		//counter ++;//
		const int pdgId = genp->pdgId();
		if (genp->status() == 1) {
			if (pdgId == 13) {
				lepMinusAll.push_back(&*genp);
			}
			if (pdgId == -13) {
				lepPlusAll.push_back(&*genp);
			}
			if (pdgId == 14 && genp->mother()->pdgId()==24) {
				neutrino = &*genp;
			}
			else if (pdgId == -14 && genp->mother()->pdgId()==24) {
				antiNeutrino = &*genp;
			}
		}
	...
	/*sizeMinus =lepMinusAll.size();
	  sizePlus =lepPlusAll.size();
	  std::cout << "size= " << sizeMinus + sizePlus << std::endl;*/
